...
VKAPI_ATTR VkResult VKAPI_CALL CreateMirSurfaceKHR(
    ...
    instance_layer_data *instance_data = GetLayerDataPtr(get_dispatch_key(instance), instance_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallCreateMirSurfaceKHR(instance, pCreateInfo, pAllocator, pSurface);
    }
    VkResult result = instance_data->dispatch_table.CreateMirSurfaceKHR(instance, pCreateInfo, pAllocator, pSurface);
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallCreateMirSurfaceKHR(instance, pCreateInfo, pAllocator, pSurface);
    }
    return result;
}
...
VKAPI_ATTR VkBool32 VKAPI_CALL GetPhysicalDeviceMirPresentationSupportKHR(
    ...
    instance_layer_data *instance_data = GetLayerDataPtr(get_dispatch_key(physicalDevice), instance_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallGetPhysicalDeviceMirPresentationSupportKHR(physicalDevice, queueFamilyIndex, connection);
    }
    ...
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallGetPhysicalDeviceMirPresentationSupportKHR(physicalDevice, queueFamilyIndex, connection);
    }
    return result;
}
...
VKAPI_ATTR VkResult VKAPI_CALL CreateAccelerationStructureNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(device), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallCreateAccelerationStructureNVX(device, pCreateInfo, pAllocator, pAccelerationStructure);
    }
    ...
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallCreateAccelerationStructureNVX(device, pCreateInfo, pAllocator, pAccelerationStructure);
    }
    return result;
}
...
VKAPI_ATTR void VKAPI_CALL DestroyAccelerationStructureNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(device), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallDestroyAccelerationStructureNVX(device, accelerationStructure, pAllocator);
    }
    device_data->dispatch_table.DestroyAccelerationStructureNVX(device, accelerationStructure, pAllocator);
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallDestroyAccelerationStructureNVX(device, accelerationStructure, pAllocator);
    }
}
...
VKAPI_ATTR void VKAPI_CALL GetAccelerationStructureMemoryRequirementsNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(device), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallGetAccelerationStructureMemoryRequirementsNVX(device, pInfo, pMemoryRequirements);
    }
    device_data->dispatch_table.GetAccelerationStructureMemoryRequirementsNVX(device, pInfo, pMemoryRequirements);
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallGetAccelerationStructureMemoryRequirementsNVX(device, pInfo, pMemoryRequirements);
    }
}
...
VKAPI_ATTR void VKAPI_CALL GetAccelerationStructureScratchMemoryRequirementsNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(device), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallGetAccelerationStructureScratchMemoryRequirementsNVX(device, pInfo, pMemoryRequirements);
    }
    device_data->dispatch_table.GetAccelerationStructureScratchMemoryRequirementsNVX(device, pInfo, pMemoryRequirements);
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallGetAccelerationStructureScratchMemoryRequirementsNVX(device, pInfo, pMemoryRequirements);
    }
}
...
VKAPI_ATTR VkResult VKAPI_CALL BindAccelerationStructureMemoryNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(device), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallBindAccelerationStructureMemoryNVX(device, bindInfoCount, pBindInfos);
    }
    VkResult result = device_data->dispatch_table.BindAccelerationStructureMemoryNVX(device, bindInfoCount, pBindInfos);
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallBindAccelerationStructureMemoryNVX(device, bindInfoCount, pBindInfos);
    }
    return result;
}
...
VKAPI_ATTR void VKAPI_CALL CmdBuildAccelerationStructureNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(cmdBuf), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallCmdBuildAccelerationStructureNVX(cmdBuf, type, instanceCount, instanceData, instanceOffset, geometryCount, pGeometries, flags, update, dst, src, scratch, scratchOffset);
    }
    device_data->dispatch_table.CmdBuildAccelerationStructureNVX(cmdBuf, type, instanceCount, instanceData, instanceOffset, geometryCount, pGeometries, flags, update, dst, src, scratch, scratchOffset);
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallCmdBuildAccelerationStructureNVX(cmdBuf, type, instanceCount, instanceData, instanceOffset, geometryCount, pGeometries, flags, update, dst, src, scratch, scratchOffset);
    }
}
...
VKAPI_ATTR void VKAPI_CALL CmdCopyAccelerationStructureNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(cmdBuf), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallCmdCopyAccelerationStructureNVX(cmdBuf, dst, src, mode);
    }
    device_data->dispatch_table.CmdCopyAccelerationStructureNVX(cmdBuf, dst, src, mode);
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallCmdCopyAccelerationStructureNVX(cmdBuf, dst, src, mode);
    }
}
...
VKAPI_ATTR void VKAPI_CALL CmdTraceRaysNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(cmdBuf), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallCmdTraceRaysNVX(cmdBuf, raygenShaderBindingTableBuffer, raygenShaderBindingOffset, missShaderBindingTableBuffer, missShaderBindingOffset, missShaderBindingStride, hitShaderBindingTableBuffer, hitShaderBindingOffset, hitShaderBindingStride, width, height);
    }
    device_data->dispatch_table.CmdTraceRaysNVX(cmdBuf, raygenShaderBindingTableBuffer, raygenShaderBindingOffset, missShaderBindingTableBuffer, missShaderBindingOffset, missShaderBindingStride, hitShaderBindingTableBuffer, hitShaderBindingOffset, hitShaderBindingStride, width, height);
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallCmdTraceRaysNVX(cmdBuf, raygenShaderBindingTableBuffer, raygenShaderBindingOffset, missShaderBindingTableBuffer, missShaderBindingOffset, missShaderBindingStride, hitShaderBindingTableBuffer, hitShaderBindingOffset, hitShaderBindingStride, width, height);
    }
}
...
VKAPI_ATTR VkResult VKAPI_CALL CreateRaytracingPipelinesNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(device), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallCreateRaytracingPipelinesNVX(device, pipelineCache, createInfoCount, pCreateInfos, pAllocator, pPipelines);
    }
    ...
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallCreateRaytracingPipelinesNVX(device, pipelineCache, createInfoCount, pCreateInfos, pAllocator, pPipelines);
    }
    return result;
}
...
VKAPI_ATTR VkResult VKAPI_CALL GetRaytracingShaderHandlesNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(device), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallGetRaytracingShaderHandlesNVX(device, pipeline, firstGroup, groupCount, dataSize, pData);
    }
    ...
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallGetRaytracingShaderHandlesNVX(device, pipeline, firstGroup, groupCount, dataSize, pData);
    }
    return result;
}
...
VKAPI_ATTR VkResult VKAPI_CALL GetAccelerationStructureHandleNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(device), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallGetAccelerationStructureHandleNVX(device, accelerationStructure, dataSize, pData);
    }
    VkResult result = device_data->dispatch_table.GetAccelerationStructureHandleNVX(device, accelerationStructure, dataSize, pData);
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallGetAccelerationStructureHandleNVX(device, accelerationStructure, dataSize, pData);
    }
    return result;
}
...
VKAPI_ATTR void VKAPI_CALL CmdWriteAccelerationStructurePropertiesNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(cmdBuf), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallCmdWriteAccelerationStructurePropertiesNVX(cmdBuf, accelerationStructure, queryType, queryPool, query);
    }
    device_data->dispatch_table.CmdWriteAccelerationStructurePropertiesNVX(cmdBuf, accelerationStructure, queryType, queryPool, query);
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallCmdWriteAccelerationStructurePropertiesNVX(cmdBuf, accelerationStructure, queryType, queryPool, query);
    }
}
...
VKAPI_ATTR VkResult VKAPI_CALL CompileDeferredNVX(
    ...
    device_layer_data *device_data = GetLayerDataPtr(get_dispatch_key(device), device_layer_data_map);
    for (auto intercept : global_interceptor_list) {
        intercept->PreCallCompileDeferredNVX(device, pipeline, shader);
    }
    VkResult result = device_data->dispatch_table.CompileDeferredNVX(device, pipeline, shader);
    for (auto intercept : global_interceptor_list) {
        intercept->PostCallCompileDeferredNVX(device, pipeline, shader);
    }
    return result;
}
