...
{
    dispatch_key key = get_dispatch_key(cmdBuf);
    layer_data *my_data = GetLayerDataPtr(key, layer_data_map);
    VkLayerDispatchTable *pTable = my_data->device_dispatch_table;
    bool threadChecks = startMultiThread();
    if (threadChecks) {
        startReadObject(my_data, cmdBuf);
    ...
    pTable->CmdBuildAccelerationStructureNVX(cmdBuf,type,instanceCount,instanceData,instanceOffset,geometryCount,pGeometries,flags,update,dst,src,scratch,scratchOffset);
    if (threadChecks) {
        finishReadObject(my_data, cmdBuf);
    ...
    VkCopyAccelerationStructureModeNVX          mode)
{
    ...
    pTable->CmdCopyAccelerationStructureNVX(cmdBuf,dst,src,mode);
    if (threadChecks) {
        finishReadObject(my_data, cmdBuf);
        finishReadObject(my_data, dst);
        finishReadObject(my_data, src);
    ...
    uint32_t                                    height)
{
    ...
    pTable->CmdTraceRaysNVX(cmdBuf,raygenShaderBindingTableBuffer,raygenShaderBindingOffset,missShaderBindingTableBuffer,missShaderBindingOffset,missShaderBindingStride,hitShaderBindingTableBuffer,hitShaderBindingOffset,hitShaderBindingStride,width,height);
    if (threadChecks) {
        finishReadObject(my_data, cmdBuf);
    ...
    uint32_t                                    query)
{
    ...
    pTable->CmdWriteAccelerationStructurePropertiesNVX(cmdBuf,accelerationStructure,queryType,queryPool,query);
    if (threadChecks) {
        finishReadObject(my_data, cmdBuf);
        finishReadObject(my_data, accelerationStructure);
        finishReadObject(my_data, queryPool);
